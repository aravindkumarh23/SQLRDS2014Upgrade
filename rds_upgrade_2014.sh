...
source_db_instance_identifier='rds2016saz'
region='us-east-2'
...
sql2019=15
sql2022=16
source_engineversion_temp=`aws rds describe-db-instances --db-instance-identifier $source_db_instance_identifier --query 'DBInstances[*].EngineVersion'|sed 's/[^0-9]//g'`
...
lis_endpoint=`aws rds describe-db-instances --db-instance-identifier $source_db_instance_identifier --query 'DBInstances[*].ListenerEndpoint.Address|[0]'|sed -r 's/"//g'`
...
#low_mzvalue_true="false"
echo "$low_mzvalue_true"
target_major_engine_version_maz=15
echo "11 $target_major_engine_version_maz"

if [[ ($lis_endpoint =~ (null)) && ($multiaz =~ ($low_mzvalue_true)) && ($target_major_engine_version_maz -eq 16) ]]; then   
...
if [[ $source_option_group_name =~ $default_og ]]; then
    ...
    py db_options.py
    aws rds add-option-to-option-group --option-group-name $target_option_group_name --options file://og_output.json>>upgrade_output.log
    date '+%Y-%m-%d %H:%M:%S'>>upgrade_output.log
...
aws rds   modify-db-instance --db-instance-identifier $source_db_instance_identifier --engine-version $target_engine_version --db-parameter-group-name $target_db_parameter_group_name  --option-group-name $target_option_group_name --allow-major-version-upgrade --apply-immediately>>upgrade_output.log
while true;
do
    sleep 01
    dbstatus=$(aws rds describe-db-instances --db-instance-identifier $source_db_instance_identifier --query 'DBInstances[*].DBInstanceStatus|[0]')
